...
#define COUNT 15
	...
	for (i=0; i<COUNT; i++)
	{
		...
		Test_ID(pcs[i], instructions[i], regs, &out[i], &lwAhead, NULL);

		// test it again, but with a NOP in the slot ahead - this
		// should have the same result.
		Test_ID(pcs[i], instructions[i], regs, &out[i], NULL,NULL);
	}
