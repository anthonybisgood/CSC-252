...
.text
	...
	lw	$a1, 0($a1)
	jal     turtle_sortByX_indirect


	# dump out the contents of the turtles (directly), to confirm
	# that the *contents* were not changed
	la      $a0, TEST_TURTLE_1
	...
	# dump out the contents of the turtles (using the indirect array),
	# to see if they got sorted correctly.
	la      $a0,  TEST_TURTLE_ARRAY
	lw      $a0,  0($a0)
	jal     turtle_debug
