...
int main()
{
	...
	// div   $s0,$s1
	// mfhi  $t0
	cpuState.instMemory[0x13e] = DIV (S_REG(0),S_REG(1));
	cpuState.instMemory[0x13f] = MFHI(T_REG(0));

	// div   $s2,$s3
	// mfhi  $t1
	cpuState.instMemory[0x140] = DIV (S_REG(2),S_REG(3));
	cpuState.instMemory[0x141] = MFHI(T_REG(1));
