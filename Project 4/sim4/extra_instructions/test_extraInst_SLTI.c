...
int main()
{
	...
	// addi  $s0, $zero, 0x2468
	// addi  $s1, $zero, 0x3579
	cpuState.instMemory[0x100] = ADDI(S_REG(0), REG_ZERO, 0x2468);
	cpuState.instMemory[0x101] = ADDI(S_REG(1), REG_ZERO, 0x3579);

	// slti  $s4, $s0, 0x7c55
	// slti  $s5, $s1, 0x1234
	cpuState.instMemory[0x102] = SLTI(S_REG(4), S_REG(0), 0x7c55);
	cpuState.instMemory[0x103] = SLTI(S_REG(5), S_REG(1), 0x1234);
